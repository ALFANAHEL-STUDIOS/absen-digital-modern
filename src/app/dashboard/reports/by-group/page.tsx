...
export default function GroupAttendanceReport() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
     ...
     // doc.text(`${schoolInfo.address}, ${currentDate}`, pageWidth - margin, yPos + 15, { align: "right" });
      ...
      doc.text("Administrator Desa", margin + signatureWidth * 1.75, yPos + 25, { align: "center" });
      ...
      const fileName = `Laporan_Kehadiran_Jabatan_${format(new Date(), "yyyyMMdd")}.pdf`;
      doc.save(fileName);
      
      toast.success(`Laporan Jabatan ${selectedClass === "all" ? "Semua Jabatan" : selectedClass} telah berhasil diunduh sebagai ${fileName}`);
    } catch (error) {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      toast.success(`Laporan Jabatan : selectedClass} berhasil diunduh sebagai ${fileName}`);
    } catch (error) {
